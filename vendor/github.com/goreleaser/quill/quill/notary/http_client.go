	...
	return &httpClient{
		client: &http.Client{
			...
			CheckRedirect: func(req *http.Request, via []*http.Request) error {
				if _, err := validator.Validate(req.URL.String()); err != nil {
...
func (s httpClient) getUnauthenticated(ctx context.Context, endpoint string) (*http.Response, error) {
	...
	if _, err := s.validator.Validate(endpoint); err != nil {
...
func (s httpClient) do(request *http.Request) (*http.Response, error) {
	// validate URL to prevent SSRF attacks
	if _, err := s.validator.Validate(request.URL.String()); err != nil {
		return nil, fmt.Errorf("URL validation failed: %w", err)
	}
